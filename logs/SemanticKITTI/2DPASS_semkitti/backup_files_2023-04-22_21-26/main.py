...
def parse_config():
    ...
    parser.add_argument('--check_val_every_n_epoch', type=int, default=1, help='check_val_every_n_epoch')#设置每多少轮验证一次
    parser.add_argument('--SWA', action='store_true', default=False, help='StochasticWeightAveraging')
    parser.add_argument('--baseline_only', action='store_true', default=False, help='training without 2D')#训练时是否使用2d数据进行辅助
    # testing
    parser.add_argument('--test', action='store_true', default=False, help='test mode')#是否使用测试模式
    parser.add_argument('--fine_tune', action='store_true', default=False, help='fine tune mode')#是否使用微调模式
    parser.add_argument('--pretrain2d', action='store_true', default=False, help='use pre-trained 2d network')#是否使用预训练的2d网络
    parser.add_argument('--num_vote', type=int, default=1, help='number of voting in the test')
    parser.add_argument('--submit_to_server', action='store_true', default=False, help='submit on benchmark')#是否上传到比赛服务器
    parser.add_argument('--checkpoint', type=str, default=None, help='load checkpoint')#加载checkpoint，主要用于接续训练和测试
    # debug
    parser.add_argument('--debug', default=False, action='store_true')#是否使用debug模式
...
def build_loader(config):
    pc_dataset = get_pc_model_class(config['dataset_params']['pc_dataset_type'])#选择数据集是semantickitti还是nuscenes
    dataset_type = get_model_class(config['dataset_params']['dataset_type'])#纯点云还是点云图像混合
    train_config = config['dataset_params']['train_data_loader']#加载训练数据的参数，主要包括batchsize、num_workers、data_path以及一些图像增强选项，默认使用图像增强
    val_config = config['dataset_params']['val_data_loader']#加载测试数据时的参数，主要包括batchsize、num_workers、data_path以及一些图像增强选项，默认不使用图像增强
    train_dataset_loader, val_dataset_loader, test_dataset_loader = None, None, None

    if not config['test']:
        train_pt_dataset = pc_dataset(config, data_path=train_config['data_path'], imageset='train')#在semantic-kitti.yaml文件中写明了数据集的划分
        ...
        train_dataset_loader = torch.utils.data.DataLoader(
            ...
            pin_memory=True,
            drop_last=True#如果数据集处以batch_size除不尽，是否丢弃掉最后的部分
        )
...
if __name__ == '__main__':
    ...
    # setting
    os.environ["CUDA_VISIBLE_DEVICES"] = ','.join(map(str, configs.gpu))#设置gpu
    num_gpu = len(configs.gpu)

    # output path
    log_folder = 'logs/' + configs['dataset_params']['pc_dataset_type']#设置保存路径
